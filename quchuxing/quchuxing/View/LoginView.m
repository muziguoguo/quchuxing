...
#warning 消除
    [_phoneNumberField setText:@"15130096570"];
    [_passwordField setText:@"666666"];
...
- (IBAction)clickedLoginBtn:(UIButton *)sender {
    ...
    [APIClient networkPostLoginWithPhone:phoneNumber captcha:verCode idfa:idfa success:^(id response) {
        ...
        if (statusCode == kSuccess) {
            [hud hideAnimated:YES];
            [[NSNotificationCenter defaultCenter] postNotificationName:LoginInNotification object:nil];
            NSError *error = nil;
            UserModel *user = [[UserModel alloc] initWithDictionary:response error:&error];
            ...
            hud.label.text = [APIClient errorMessageWithStatusCode:statusCode];
